...
class App(ct.CTk):
    def __init__(self):
                        ...
                        if(check_var1 ==0 and check_var1 == 0 and check_var2 == 0 and check_var3 == 0 and check_var4 == 0):
                            ...
                            def check_checkbuttom_vals():
                                ...
                                if(check_var4.get()):
                                    ...
                                    pdf.multi_cell(w=0,txt="CALL DURATION REPORT",border=1,h=10,new_x=XPos.LMARGIN,new_y=YPos.NEXT)
                                    pdf.multi_cell(w=0,txt =output,border=1,h=10,new_x=XPos.LMARGIN,new_y=YPos.NEXT)
        ...
        def open_srtp():
                ...
                filename = askopenfilename(title="Select the capture file",filetypes=[("Capture files", ".pcap .pcapng")])
                self.srtp_pcap_file_label.configure(text="File Opened: "+str(filename))
        ...
        self.map_widget.grid(row=6,rowspan = 6,columnspan=6,sticky="nswe")
